...
class Init{
    ...
    init() {
        ...
        if(version <= 15){
            ...
            addSong(portable1, "Ask to Wind ~Live Mix~".localized, "Forte Escape", "210", 5, 0, 12, 6, 10, 0, 7, 11, 0, 7, 0, 13)
        ...
        if version <= 207 {
            ...
            initRecord(portable1, "Ask to Wind ~Live Mix~".localized)
            initRecordDifficulty("Ask to Wind ~Live Mix~".localized, 5, 6, 7, 7, 0, 10, 11, 0, 12, 0, 0, 13)
            //lowercase 초기화
            let songResults = realm.objects(SongInfo.self)
            let recordResults = realm.objects(RecordInfo.self)
            for index in 0..<songResults.count {
