...
extension RecordBaseTableViewController {
    private func setRank(_ object: NewRecordInfo, rank: String, difficulty: String, button: String) {
        guard let buttonExpansion = button.buttonExpansion else { return }
        let keyPath = "\(buttonExpansion).\(difficulty).rank"
        NewRecordInfo.update(object, with: [keyPath: rank])
    }
    
    private func setRate(_ object: NewRecordInfo, rate: Double, difficulty: String, button: String) {
        guard let buttonExpansion = button.buttonExpansion else { return }
        let keyPath = "\(buttonExpansion).\(difficulty).rate"
        NewRecordInfo.update(object, with: [keyPath: rate])
        setSkillPoint(object, button: button)
        recordView.updateRankingAndSkillPointLabel(object, button: button)
    }
    
    private func setNote(_ object: NewRecordInfo, note: String, difficulty: String, button: String) {
        guard let buttonExpansion = button.buttonExpansion else { return }
    ...
    private func setSkillPoint(_ object: NewRecordInfo, button: String) {
        ...
        guard let songResult = self.tempSongResults?.filter(predicate).first else { return }
        guard let songButtonKeyPath = songResult.value(forKeyPath: buttonExpansion) as? SongButtonInfo else { return }
        let normal = Skill.skillPoint(difficulty: songButtonKeyPath.normal, rate: recordButtonKeyPath.normal?.rate, note: recordButtonKeyPath.normal?.note)
        let hard = Skill.skillPoint(difficulty: songButtonKeyPath.hard, rate: recordButtonKeyPath.hard?.rate, note: recordButtonKeyPath.hard?.note)
        let maximum = Skill.skillPoint(difficulty: songButtonKeyPath.maximum, rate: recordButtonKeyPath.maximum?.rate, note: recordButtonKeyPath.maximum?.note)
        guard let max = [normal, hard, maximum].sorted().last else { return }
        NewRecordInfo.update(object, with: ["\(buttonExpansion).skillPoint": max])
        switch max {
        case normal:
            NewRecordInfo.update(object, with: [
